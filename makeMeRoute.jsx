...
var Atlanta = React.createClass({
	...
	render: function(){
		var iconToPrint = 'http://openweathermap.org/img/w/' + this.state.icon + '.png';
		return(
			<div>
				<h1>Real-time Atlanta Weather!</h1>
				<p>The temp is: {this.state.temp}</p>
				<p>The max temp is: {this.state.temp_max}</p>
				<p>The min temp is: {this.state.temp_min}</p>
				<p>The icon is: <img src={iconToPrint}/></p>
				<p>The description is: {this.state.desc}</p>
			</div>
		);
	}
});
